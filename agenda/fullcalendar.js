...
Grid.mixin({
	...
	buildSegResizeListener: function(seg, isStart) {
		...
		var dragListener = this.segResizeListener = new HitDragListener(this, {
			...
			interactionEnd: function(ev) {
				...
				if (resizeLocation) { // Fecha válida para cambiar el tamaño a?
					// no need to re-show original, will rerender all anyways. esp important if eventRenderWait
					// Sin necesidad de volver a mostrar original, rerender todos de todos modos. Especialmente importante si eventRenderWait
					view.reportEventResize(event, resizeLocation, _this.largeUnit, el, ev);
					
					//UPDATE EN DDBB el cambio de horario
					updateSqlEvents(event);
	...
	//Devuelve nueva información de fecha para un segmento de evento que se redimensiona desde su final
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	title: null, // El texto que se mostrará en el título del encabezado
...
function EventManager() { // assumed to be a calendar
	...
	// Sólo se llama desde la API externa
	...
	function updateEvents(events) {
		...
		for (i = 0; i < events.length; i++) {
			...
			// Valores de inicio / fin de masaje, incluso si son valores de cadena de fecha
			...
			mutateEvent(event, getMiscEventProps(event)); // Controlará la normalización start / end / allDay
		}

		reportEventChange(); // Informes de modificaciones de eventos (para que podamos redibujar)
	}
...
// Métodos que personalizan el comportamiento de representación de la agendaGrid de CalendarView
// TODO: mover a TimeGrid
...
//ver setEvents y updateEvensts por 
function updateSqlEvents(event){
	var eventData;
	eventData = {
		id		:	event.id,
		title	:	event.title,
		//texto	:	"asd",
		color	:	event.color,
		start	:	event.start.format("MM/DD/Y HH:mm:ss"),
		end		:	event.end.format("MM/DD/Y HH:mm:ss"),
	...
	$.ajax({
		...
		success: function(result){	
			if(isNaN(result)){
				alert("Hubo un error al intentar modificar el evento."); 
			}
		},
		error: function () {
