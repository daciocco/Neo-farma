...
function dac_cargarDatosCuenta(ctaId, empresa, idCuenta, zonaV, stringZonas) {
	...
	var campo =	'<div id="rutcuenta'+nextCuenta+'">';
		campo +='<div class="bloque_8"><input id="ctaId'+nextCuenta+'" name="ctaId[]" type="text" value='+ctaId+' hidden="hidden"><input type="text" value='+empresa+' disabled></div><div class="bloque_7"><input type="text" value='+idCuenta+' disabled ></div>';
		//------------
		//	ZONAS	
		if(stringZonas){ var zonas	= stringZonas.split(","); }
		campo += '<div class="bloque_8"><select name="zonaVExc[]">';
		...
		campo += '</select></div>';
		//------------			
		campo +='<div class="bloque_9"><input id="btmenos" class="btmenos" type="button" value=" - " onClick="dac_deleteCuenta('+ctaId+', '+nextCuenta+')"></div>';	
	campo +='</div><hr>';
